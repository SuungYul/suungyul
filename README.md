...
<p align="center">
제품 하나의 <b>DB·API·데이터 파이프라인·인프라</b>를 직접 설계하고 운영하는 백엔드·데이터 엔지니어입니다.<br/>
...
- 현재 GC(녹십자홀딩스) 커머셜솔루션팀에서 제약·헬스케어 B2B 제품의 DB·API·데이터 배치·인프라를 설계하고 운영합니다.
...
- **백엔드 성능 엔지니어링** — API 6분 → 0.2초, 핵심 쿼리 92초 → 0.3초, 분석 롤업 46초 → 0.02초
- **세그먼트 분석 엔진** — RFM과 장바구니 연관분석으로 교차판매 타겟을 도출하고 캠페인 조건빌더로 연결
- **이종 도메인 확장** — 단일 도메인 플래그로 매출 중심 제품을 검사건수 중심 도메인까지 확장하고 약 1,300만 행 파이프라인 구성
- **대용량 업로드 파이프라인** — 병원 자동매핑 150초 → 7초, NDJSON 스트리밍 진행률, PII 스캔·차단 적용
- **꾹 공공데이터 정합성 개선** — 행정구역 개편에 따른 공공데이터 간 갱신 시차를 좌표 기반 재매핑으로 보정
- **자체 크롤링 파이프라인** — 외부 솔루션을 무중단 전환·즉시 롤백 구조로 완전 대체, 86분 → 13분 및 연 1,200만원 절감
- **인프라 현대화** — 사내 최초 GitLab CI/CD 도입, AWS EC2 ARM·RDS Blue/Green 무중단 이관

### Representative Decision

- **문제 정의** — 병원 업로드 자동매핑이 150초까지 걸려 60초 게이트웨이 제한을 넘었고, 사용자는 정상 처리 중인 작업도 실패로 인식했습니다.
- **의사결정** — 타임아웃만 늘리지 않고 ID 인덱스·지역 선분할로 매칭 비용을 먼저 줄인 뒤, NDJSON 스트리밍으로 실제 진행률을 전달했습니다.
- **결과** — 자동매핑을 150초 → 7초(21배)로 단축하고, 더 긴 작업도 진행 상태를 확인하며 완료할 수 있게 했습니다.
